...
async def combat_query(message, channel, weapon):
	...
	combat_messages_delete.extend(message_group)
	...
	for i in message_group:
		if i.content.split()[2] == "start":
			if already_fighting:
				...
				flags[channel] = True
				await combat_clear_messages(channel, message_group, combat_messages_delete)
